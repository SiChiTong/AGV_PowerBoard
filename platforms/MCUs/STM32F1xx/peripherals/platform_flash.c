...
static OSStatus interna_flash_erase(uint32_t StartAddress, uint32_t EndAddress);
static OSStatus internal_flash_write(volatile uint32_t* FlashAddress, uint32_t* Data ,uint32_t DataLength);
//static OSStatus internalFlashByteWrite(volatile uint32_t* FlashAddress, uint8_t* Data ,uint32_t DataLength);
static OSStatus internal_flash_protect(uint32_t StartAddress, uint32_t EndAddress, bool enable);
static uint32_t get_wrp_sector(uint32_t Address);
static uint32_t get_page_address(uint32_t Address);
...
OSStatus platform_flash_erase(const platform_flash_t *peripheral, uint32_t start_address, uint32_t end_address)
{
    ...
    if(peripheral->flash_type == FLASH_TYPE_EMBEDDED){
        err = interna_flash_erase(start_address, end_address);
        require_noerr(err, exit);
    }
#ifdef USE_MICO_SPI_FLASH
    else if(peripheral->flash_type == FLASH_TYPE_SPI){
...
OSStatus platform_flash_write(const platform_flash_t *peripheral, volatile uint32_t* start_address, uint8_t* data ,uint32_t length)
{
    ...
    if(peripheral->flash_type == FLASH_TYPE_EMBEDDED){
        err = internal_flash_write(start_address, (uint32_t *)data, length);
        require_noerr(err, exit);
    }
#ifdef USE_MICO_SPI_FLASH
    else if(peripheral->flash_type == FLASH_TYPE_SPI){
...
OSStatus platform_flash_enable_protect(const platform_flash_t *peripheral, uint32_t start_address, uint32_t end_address)
{
    ...
    if(peripheral->flash_type == FLASH_TYPE_EMBEDDED){
        ...
        err = internal_flash_protect(start_address, end_address, true);
#endif
        require_noerr(err, exit);
    }
#ifdef USE_MICO_SPI_FLASH
...
OSStatus platform_flash_disable_protect(const platform_flash_t *peripheral, uint32_t start_address, uint32_t end_address)
{
    ...
    if(peripheral->flash_type == FLASH_TYPE_EMBEDDED){
        err = internal_flash_protect(start_address, end_address, false);
        require_noerr(err, exit);
    }
#ifdef USE_MICO_SPI_FLASH
    else if(peripheral->flash_type == FLASH_TYPE_SPI){
...
OSStatus interna_flash_erase(uint32_t StartAddress, uint32_t EndAddress)
{
    platform_log_trace();
    OSStatus err = kNoErr;
    ...
    StartPageAddredss = get_page_address(StartAddress);
    EndPagesAddress = get_page_address(EndAddress);
...
OSStatus internal_flash_write(volatile uint32_t* FlashAddress, uint32_t* Data ,uint32_t DataLength)
{
    platform_log_trace();
    OSStatus err = kNoErr;
    uint32_t i = 0;
...
OSStatus internal_flash_protect(uint32_t StartAddress, uint32_t EndAddress, bool enable)
{
    OSStatus err = kNoErr;
    ...
    StartSector = get_wrp_sector(StartAddress);
    EndSector = get_wrp_sector(EndAddress);
