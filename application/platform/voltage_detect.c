...
static voltage_data_t ramVoltageConvert;
voltage_data_t *voltageConvert = NULL;//&ramVoltageConvert;

#ifdef VOLTAGE_DEBUG
static voltage_data_t ramTempMaxVoltageData;
static voltage_data_t *tempMaxVoltageData = NULL;
#endif

voltage_convert_data_t *voltageConvertData;
voltage_debug_t voltageDebug;
...
OSStatus detect_vol_init(void)
{
    ...
    memset(voltageConvert, 0x0, sizeof(voltage_data_t));

#ifdef VOLTAGE_DEBUG
    tempMaxVoltageData = &ramTempMaxVoltageData;
    memset(tempMaxVoltageData, 0x0, sizeof(voltage_data_t));
    if(tempMaxVoltageData)
    {
        tempMaxVoltageData->bat_voltage = 10000;//set temp max voltage is 100v
    }
#endif
    voltageConvertData = (voltage_convert_data_t *)malloc(sizeof(voltage_convert_data_t));
    require_action(voltageConvertData, exit, err = kNoMemoryErr);
    memset(voltageConvertData, 0x0, sizeof(voltage_convert_data_t));
...
static uint16_t processChannelsData(adc_channel_t type)
{
    ...
    read_data = (uint16_t)(read_channel_values_mV(pConvertAdcData->adc_type) * pConvertAdcData->convert_factor);

    if(read_data > pConvertAdcData->threshold_high || read_data < pConvertAdcData->threshold_low)
    {
        ...
        else
        {
                    ...
                    pConvertAdcData->err_duration_time)
            {
                *(uint32_t *)voltageConvertData->faultBitTemp |= \
                ...
                pConvertAdcData->err_value = read_data;
                boardStatus->err_channel = pConvertAdcData->err_channel;
                boardStatus->err_value = read_data;
            }
        }
    }
    else
...
static void computeVoltage(void)
{
    //boardStatus->bat_level = get_percentage_from_battery_voltage(voltageConvert->bat_voltage);
...
void PrintAdcData(void)
{
    ...
    for(uint8_t i = 1; i <= sizeof(voltage_data_t)/2; i++)
    {
        if(i % 10 == 0)
        {
            printf("\r\n");
...
void detect_vol_tick(void)
{
    ...
    if((YES == voltageDebug.isNeedUpload) && IS_SEND_RATE_DATA(voltageDebug.uploadRate))
    {
        if(voltageDebug.uploadRate == SEND_RATE_SINGLE)
        {
            ...
            boardStatus->system_status &= ~(uint16_t)STATE_IS_AUTO_UPLOAD;
        }
        else
        {
            if((os_get_time() - voltageDebug.uploadFlagTime) >= map_time_by_rate(voltageDebug.uploadRate))
            {
                boardStatus->system_status |= (uint16_t)STATE_IS_AUTO_UPLOAD;
...
void battery_percentage_1s_period(void)
{
    ...
    if(!(boardStatus->system_status & (uint16_t)STATE_IS_CHARGING))
    {
        if(minBatLevel < boardStatus->bat_level)
        {
            boardStatus->bat_level = minBatLevel;
        }
    }
    else
    {
    ...
    //vol_detect_log("bat_level is %d", boardStatus->bat_level);
}
#endif
